...
class DashboardUIController: NSObject {
    ...
    private func setupImagePickerController() {
        ...
        imagePickerController.mediaTypes = [AppConstant.imageType]
    }
    ...
    private func setupTableView() {
        self.view?.tableView.register(UINib(nibName: AppConstant.carTableViewCell, bundle: nil), forCellReuseIdentifier: AppConstant.carTableViewCell)
    ...
    private func setupTextFields() {
        ...
        if self.view?.selectMakeDropDowntextField.text?.count ?? 0 < 1 {
            self.view?.selectMakeDropDowntextField.text = AppConstant.selectMake
        }
        if self.view?.selectModelDropDownTextField.text?.count ?? 0 < 1 {
            self.view?.selectModelDropDownTextField.text = AppConstant.selectModel
        }
    }
    ...
    @objc private func didTapOnLogoutButton() {
        let alert = UIAlertController(title: AppConstant.alert, message: AppConstant.logoutMsg, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: AppConstant.okTitle, style: .default, handler: { _ in
            UserManager.shared.currentLoggedInUserName = nil
        ...
        alert.addAction(UIAlertAction.init(title: AppConstant.cancelTitle, style: .cancel, handler: { _ in
    ...
    @objc private func didTapOnSelectModelTextField() {
        if self.view?.selectMakeDropDowntextField.text == AppConstant.selectMake {
            (self.view as? UIViewController)?.showAlert(title: AppConstant.alert, message: AppConstant.selectMakeErrorMsg, comletion: nil)
        } else {
    ...
    @objc private func didTapOnAddCarButton() {
        if self.view?.selectMakeDropDowntextField.text == AppConstant.selectMake && self.view?.selectModelDropDownTextField.text == AppConstant.selectModel {
            (self.view as? UIViewController)?.showAlert(title: AppConstant.alert, message: AppConstant.selectMakeAndModelErrorMsg, comletion: nil)
        } else if self.view?.selectMakeDropDowntextField.text != AppConstant.selectMake && self.view?.selectModelDropDownTextField.text == AppConstant.selectModel {
            (self.view as? UIViewController)?.showAlert(title: AppConstant.alert, message: AppConstant.selectModelErrorMsg, comletion: nil)
        } else {
            CarManager.shared.saveCarToDB(car: Car(make: self.makeName ?? "", model: self.modelName ?? "", image: UIImage()))
            self.view?.selectMakeDropDowntextField.text = AppConstant.selectMake
            self.view?.selectModelDropDownTextField.text = AppConstant.selectModel
            (self.view as? UIViewController)?.showToast(message: AppConstant.carAddToastMsg)
...
extension DashboardUIController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: AppConstant.carTableViewCell, for: indexPath) as? CarTableViewCell else { return UITableViewCell() }
        cell.delegate = self
...
extension DashboardUIController: DashboardDropDownDataFetched {
    func error(message: String) {
        ...
        (self.view as? UIViewController)?.showAlert(title: AppConstant.errorTitle, message: message, comletion: nil)
    }
...
extension DashboardUIController: AddCarImageDelegate {
    ...
    private func openCamera() {
        if(UIImagePickerController .isSourceTypeAvailable(UIImagePickerController.SourceType.camera)) {
            ...
            let alert  = UIAlertController(title: AppConstant.warningTitle, message: AppConstant.noCameraErrorMsg, preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: AppConstant.okTitle, style: .default, handler: nil))
            (self.view as? UIViewController)?.present(alert, animated: true, completion: nil)
        }
    }
    ...
    private func choosePhoto() {
        let alert = UIAlertController(title: AppConstant.chooseImage, message: nil, preferredStyle: .actionSheet)
        alert.addAction(UIAlertAction(title: AppConstant.camera, style: .default, handler: { _ in
            self.openCamera()
        }))
        alert.addAction(UIAlertAction(title: AppConstant.gallery, style: .default, handler: { _ in
            self.openGallery()
        }))
        alert.addAction(UIAlertAction.init(title: AppConstant.cancelTitle, style: .cancel, handler: nil))
        (self.view as? UIViewController)?.present(alert, animated: true, completion: nil)
    }
